...
<div class="w-full h-main-content-area max-h-main-content-area text-white relative">
	...
	{#if loaded}
					...
					<div class="  my-auto pb-10 w-full dark:text-gray-100">
						...
						{#if Object.keys($config?.oauth?.providers ?? {}).length > 0}
							<div class="inline-flex items-center justify-center w-full">
								<hr class="w-12 h-px my-4 border-0 dark:bg-gray-100/10 bg-gray-700/10" />
								<!-- {#if $config?.features.enable_login_form || $config?.features.enable_ldap} -->
								<!-- 	<span -->
								...
								<span
									class="px-3 text-sm font-medium text-gray-900 dark:text-white bg-transparent"
									>{$i18n.t('Please use Globus to sign in')}</span
								>
								<hr class="w-12 h-px my-4 border-0 dark:bg-gray-100/10 bg-gray-700/10" />
							</div>
		...
		>
			<div class="flex space-x-4 items-center">
				...
				<img id="globus-logo" src="/static/globus.png" alt="Globus" class="size-18" />
			</div>
		</div>
	{/if}
</div>
